...
macOS 会复用用户当前打开的 Google Chrome，并在现有窗口的标签页中完成登录；Windows 会复用门户专用 Chrome，仅在首次运行时启动；不要为每次登录另外启动浏览器。Linux 使用 CLI 内置的浏览器兼容方案。
...
该命令会显示石景山、成华和自定义网站的可登录账号。按下 `1-9` 或第 10 项起的 `A-Z` 即可直接登录，无需回车；超过 35 个账号时改为完整序号输入。用户已经指定菜单项时，直接执行 `portal-dev login <序号或按键>`，例如 `portal-dev login 1`、`portal-dev login 10` 或 `portal-dev login A`，不要再次等待交互选择。
...
登录已配置的普通网站（账号配置 `code: true` 时自动识别图形验证码；不走 Token 跳转）：
