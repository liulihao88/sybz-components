...
macOS 会直接复用当前打开的 Google Chrome，在现有窗口中新建标签页完成登录和本地联调。Windows 首次运行时会启动一个门户专用 Chrome，之后的登录会持续复用该 Chrome 的窗口和登录状态，只新建标签页，不会每次创建新的浏览器实例。Linux 使用独立浏览器实例作为兼容方案。

Windows 会优先读取 `CHROME_PATH`，然后自动查找 Chrome 和 Microsoft Edge 的常见安装位置。门户专用 Chrome 数据保存在 `%APPDATA%\sybz-components\chrome-profile`，默认使用本机端口 `9229` 复用连接，可通过 `PORTAL_CHROME_DEBUG_PORT` 修改。配置文件优先使用 `%APPDATA%`；自动启动前端项目时通过 `cmd.exe` 运行 npm、pnpm 或 Bun，结束时会清理对应的进程树。
...
第 10-35 项也兼容菜单中的 `A-Z` 快捷键写法，例如 `portal-dev login A` 等同于 `portal-dev login 10`。
...
不传选择参数时，前 9 项使用 `1-9`，第 10 项起使用 `A-Z`，最多支持 35 个账号单键选择。超过 35 个账号或当前终端不支持单键输入时，自动改为输入完整序号后回车。
...
配置时输入登录页 URL、是否需要图形验证码、用户名和密码。默认只查找常见的用户名、邮箱、密码输入框，以及“登录”、“Login”或“Sign in”按钮；按钮文字中包含用于排版的空格时（例如“登 录”）也能识别。自定义网站不会执行样板间搜索或 Token 跳转。
...
自定义网站账号的 `code` 属性控制图形验证码登录，类型为布尔值，默认 `false`：

| 值              | 登录行为                                                                                  |
| --------------- | ----------------------------------------------------------------------------------------- |
| `false`（默认） | 填写用户名和密码后提交，不识别图形验证码。省略该属性效果相同。                            |
| `true`          | 查找可见的验证码图片和输入框，识别后填写用户名、密码及验证码并提交；失败时最多重试 5 次。 |

运行 `portal-dev config --portal custom` 时，回答“是否需要图形验证码”为 `y` 会保存 `"code": true`，直接回车则保存 `false`。验证码识别支持常见的 `captcha`、`verify`、`code` 命名及“验证码”标签，OCR 支持 3–8 位英数字图形码。
