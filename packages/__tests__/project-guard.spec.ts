...
describe('component entry guards', () => {
  ...
  it('keeps radio button colors and customLabel aligned across implementation, types and docs', () => {
    ...
    const shijingshanTheme = readText('packages/styles/themes/shijingshan/radio.scss')
    ...
    expect(radio).toContain('color: `var(--s-ch-${type})`')
    expect(radio).toContain('color: `var(--s-sjs-${type})`')
    expect(radio).toContain('color: `var(--el-color-${type})`')
    expect(radio).toContain("'--s-radio-button-hover-bg': typeColors.hoverBg")
    expect(radio).toContain('mergedProps.value.customLabel({')
    expect(radio).toContain(':class="item.class"')
    expect(radio).toContain('v-bind="getOptionAttrs(item)"')
    expect(radio).toContain(':key="getOptionKey(item, index)"')
    expect(radio).toContain('if (isDisabled.value || option.disabled) return true')
    ...
    expect(componentProps).toContain('type?: SRadioItemType')
    expect(componentProps).toContain('class?: string | string[] | Record<string, boolean>')
    expect(componentProps).toContain('style?: CSSProperties')
    ...
    expect(chenghuaTheme).toContain(':where(.s-radio-box.s-radio-box--chenghua)')
    expect(chenghuaTheme).toContain(':where(.el-radio-button__original-radio:checked)')
    expect(chenghuaTheme).toContain('var(--s-radio-button-color, var(--s-ch-primary))')
    expect(chenghuaTheme).toContain('var(--s-radio-button-hover-bg, var(--s-ch-primary-soft))')
    expect(shijingshanTheme).toContain(':where(.s-radio-box.s-radio-box--shijingshan)')
    expect(shijingshanTheme).toContain(':where(.el-radio-button__original-radio:checked)')
    expect(shijingshanTheme).toContain('var(--s-radio-button-color, var(--s-sjs-primary))')
    expect(shijingshanTheme).toContain('var(--s-radio-button-hover-bg, var(--s-sjs-primary-soft))')
    expect(docs).toContain('### 高级写法：动态 label 和选项样式')
    expect(demo).toContain('show-type="button"')
    expect(demo).toContain('const statusOptions = computed(() => {')
    expect(demo).toContain('pendingCount.value')
    expect(demo).toContain("class: ['status-radio', 'pending-radio']")
    expect(demo).toContain("type: 'danger'")
  })
