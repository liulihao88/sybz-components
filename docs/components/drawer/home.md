...
<el-tag type="danger">
由于drawer和dialog的api和用法几乎一致, 所以当前drawer组件将不再维护, 如果想使用drawer抽屉组件, 可以直接使用&lt;s-dialog type="drawer" /&gt;
</el-tag>
...
- 当前 `drawer` 组件已不再继续维护，推荐优先使用 `dialog` 组件或 `s-warning type="drawer"` 进行替代提示。
