...
`document-preview` 通过 `docx-preview` 和 `xlsx` 在浏览器端渲染 Word、Excel 文件。使用 Word 或 Excel 预览功能时，请先在项目中安装依赖：
...
组件会根据 `type` 选择预览方式：默认值为 `pdf`，但文件扩展名为 `.doc/.docx/.xls/.xlsx/.xlsm` 时会自动使用对应渲染；也可以显式传入 `type="auto"`、`type="pdf"`、`type="word"` 或 `type="excel"`。Word 和 Excel 文件地址必须允许当前页面跨域请求。
