...
## 6. 为什么项目要使用 pnpm？有什么好处，如何安装？

pnpm 是用于安装依赖、运行项目脚本的包管理器。前端项目推荐使用 pnpm，尤其是团队成员较多、需要维护多个项目或多个包时。团队统一使用 pnpm，并提交 `pnpm-lock.yaml`，可以让大家安装到相同的依赖版本，减少环境差异。

pnpm 的主要好处是：

- **节省磁盘空间**：相同依赖保存在共享的内容寻址存储中，多个项目可以复用。
- **安装更快**：复用已下载的依赖，减少重复下载和复制。
- **依赖关系更明确**：默认只允许项目直接访问已声明的依赖，有助于及早发现漏装的包。
- **方便管理多个包**：workspace 可以统一管理应用、组件库和工具包，并共用一份锁文件。

先安装 Node.js，然后用 npm 安装 pnpm，并检查安装结果：
...
安装完成后，进入使用 pnpm 的项目目录，即可安装项目依赖或添加新依赖：
...
如果使用的 Node.js 版本不支持最新版 pnpm，或需要其他安装方式，请查看 [pnpm 官方安装文档](https://pnpm.io/installation/)。
