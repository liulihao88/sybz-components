...
const TYPED_COMPONENT_PROPS = new Map([
  ...
  [
    'SPopconfirm',
    {
      ...
      slots: ['default', 'content', 'footer', 'target'],
  ...
  [
    'SMenu',
    {
      ...
      typeName: 'SMenuProps',
      description:
        's-menu 递归菜单组件，支持路由、图标、分组、默认展开、header/footer 头尾配置（支持响应式对象、handler 点击回调、class/style 和折叠悬浮文案）以及由 variant 控制的明暗外观和由 theme 控制的强调色（light 始终为白色背景）。头部和底部 padding 保持 16px，列表展开时为 16px、收起时为 4px；autoHeight 从顶部排列菜单，仅空间不足时压缩菜单项尺寸。',
      slots: ['header', 'footer'],
      instanceMembers: [
...
const ELEMENT_WRAPPER_CONFIGS = {
  ...
  table: {
    ...
    description:
      'Element Plus Table；s-table 自有属性包括主题、背景色、disabled（禁用分页和操作按钮）、分页、选择和列配置。',
  },
  tooltip: {
